function MC_MAP4SL_extracontrols(cfg,filename,outputfolder)
...
%Saskia, 22.08.22 - use this line instead
seedstring = [num2str(cfg.seed(1)),'__',num2str(cfg.seed(2)),'__',num2str(cfg.seed(3))];
...
sim=sim(1:accfactor);
[sim,idx] = sort(mod(sim,size(nii.img,3))); % this is finding the remainder after dividing sim by size(nii.img,3) and sorting that. 
% Saskia, 22.08.22 - sim now contains both the seed location and the
% possible artifact location(s), relative to the z-axis.
pp=-accfactor:accfactor;
xr = pp((accfactor+1)-(find(idx==1)-1):(accfactor+1)-(find(idx==1)-1)+accfactor-1);
% Saskia, 22.08.22 - this tells you which of the values in sim is the seed
% location and which is the possible artifact location.
...
%Extra control regions MMC 24/03/2022

if strcmp(cfg.extracontrolsflag,'yes')
    
    ii=length(simc);
    
    % extra controls with seed slice+/-3, y 1/3 and 2/3 between seed and
    % artefact and x +/-2   
 
%     step = abs(x(1)-x(2))/3;
%     extra_c_x = [min(x(1),x(2))+step min(x(1),x(2))+2*step];
%     extra_c_x = round(mod(extra_c_x, size(nii.img,2)));
%         
%     simc = [simc sim(1)+3 sim(1)-3];
%     LRc = [LRc LR+2 LR-2];
%     x3 = [x3 extra_c_x];
%     
%     for(mmc=1:2)    
%         controlpositions{ii+mmc}=[LRc(ii+mmc) x3(ii+mmc) simc(ii+mmc)];
%     end
%     
    
%    % extra controls with slice evenly spaced between seed and artefact
  
%     step = abs(sim(1)-sim(2))/2;
%     extra_c_slice = [min(sim(1),sim(2))+step min(sim(1),sim(2))+3*step];
%     extra_c_slice = round(mod(extra_c_slice, size(nii.img,3)));
%     
%     step = abs(x(1)-x(2))/3;
%     extra_c_x = [min(x(1),x(2))+step min(x(1),x(2))+2*step];
%     extra_c_x = round(mod(extra_c_x, size(nii.img,2)));    
...
%     end
    ...
    %  extra controls from specified coordinates
    for (mmc=1:size(cfg.extracontrols,1))  
        simc = [simc cfg.extracontrols(mmc,3)];
        LRc = [LRc cfg.extracontrols(mmc,1)];
        x3 = [x3 cfg.extracontrols(mmc,2)];
        controlpositions{ii+mmc}=[LRc(ii+mmc) x3(ii+mmc) simc(ii+mmc)];
    end
end
...
mmc_counter=0; %added by MMC 02/03/2022

for ii=1:length(sim)
    
    mmc_counter=mmc_counter+1; %added by MMC 02/03/2022
    
    mask(LR, x(ii), sim(ii)) = mmc_counter;  % modified by MMC02/03/2022; give different pixel values to each artefact ROI
    ...
    if strcmp(cfg.Grappa, 'yes')
        ...
        mask(LR, x2(ii), sim(ii)) = mmc_counter; % modified by MMC02/03/2022; give different pixel values to each artefact ROI
        mmm = zeros(S(1),S(2),S(3));
...
for ii=1:length(simc)
    ...
    controlmask(LRc(ii), x3(ii), simc(ii)) = ii; % modified by MMC02/03/2022; give different pixel values to each control ROI
...
% added by MMC 02/03/2022
% restore pixel value 1 for all control and artefact masks
for ii=1:length(simc)
...
% Correlations fail for ME data, so comment for now 02/03/2022
% % ---------------------------------------------------------------
% % plot correlations
% % ---------------------------------------------------------------
% 
