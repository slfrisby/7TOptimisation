function [subcode] = firstlevel_glm_mvpa(ids,sm)
...
for s=1:size(subcode,2)
    ...
    for c=1:size(cond,2)
        ...
        if exist([root,'/halaiprep/',subcode{s},'/func/',subcode{s},'_acq-',cond{c},'_motion.txt'])
            ...
            outdir=([root,'GLM/first_mni_mvpa/',subcode{s},'/',num2str(sm),'sm_',cond{c},'/'])
            datadir=([root,'SPM/first_mni_mvpa/',subcode{s}]);
            mkdir(outdir);
            mkdir(datadir);
            ...
            % no need to write contrast images - we only need beta images
            spm_jobman('run',matlabbatch);
            ...
            if c >= 4
                cd(root) 
                outdir=([root,'/GLM/first_mni_mvpa/',subcode{s},'/',num2str(sm),'sm_',cond{c},'_dn/'])
                mkdir(outdir);
                delete([outdir,'/SPM.mat']);
                ...
                matlabbatch{2}.spm.stats.fmri_est.write_residuals = 0;
                matlabbatch{2}.spm.stats.fmri_est.method.Classical = 1; % as opposed to Bayesian
                % no need to write contrast images - we only need beta
                % images
                spm_jobman('run',matlabbatch);
            ...
            if c == 2 || c == 5
                cd(root)
                outdir=([root,'GLM/first_mni_mvpa/',subcode{s},'/',num2str(sm),'sm_',cond{c},'_odd/'])
                datadir=([root,'SPM/first_mni_mvpa/',subcode{s}]);
                mkdir(outdir);
                mkdir(datadir);
                delete([outdir,'SPM.mat']);
                ...
                if c == 5
                    cd(root) 
                    outdir=([root,'/GLM/first_mni_mvpa/',subcode{s},'/',num2str(sm),'sm_',cond{c},'_dn_odd/'])
                    mkdir(outdir);
                    delete([outdir,'/SPM.mat']);
