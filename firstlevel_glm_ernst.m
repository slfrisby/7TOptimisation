function [subcode] = firstlevel_glm_ernst(ids,sm)
...
cond={'SESB','SEMB','SESBernst','SEMBernst'};
...
for s=1:size(subcode,2)
    ...
    for c=1:size(cond,2)
        ...
        if exist([root,'/halaiprep/',subcode{s},'/func/',subcode{s},'_acq-',cond{c},'_motion.txt'])
            ...
            outdir=([root,'GLM/first_mni_ernst/',subcode{s},'/',num2str(sm),'sm_',cond{c},'/'])
            datadir=([root,'SPM/first_mni_ernst/',subcode{s}]);
            mkdir(outdir);
            mkdir(datadir);
            ...
            if isempty(dir([datadir,'/s',num2str(sm),'*',cond{c},'*','.nii']))
                ...
                tmp=dir(['./halaiprep/',subcode{s},'/func/',subcode{s},'_acq-',cond{c},'_*_bold.nii.gz']);
            ...
            outdir=([root,'GLM/first_mni_ernst_mvpa/',subcode{s},'/',cond{c},'/'])
            datadir=([root,'SPM/first_mni_ernst_mvpa/',subcode{s}]);
            mkdir(outdir);
            mkdir(datadir);
            delete([outdir,'SPM.mat']);
